/**
 * Simple program demonstrating shared memory in POSIX systems.
 * This is the consumer process
 *
 * @author Agustín J. González 
 * One line at the time is read from stdin by producer and written in the shared memory.
 * Then the consumer reads each line and displays it.
 * This a variant of a very similar program by Gagne, Galvin, Silberschatz
 * Operating System Concepts - Ninth Edition
 * Copyright John Wiley & Sons - 2013
 *
 * modifications by dheller@cse.psu.edu, 31 Jan. 2014
 * modifications by Agustín J. González, 14 Oct. 2019
 *    gcc consumer.c -lpthread -lrt
 */

#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
#include <fcntl.h>
#include <sys/shm.h>
#include <sys/stat.h>
#include <sys/mman.h>
#include <errno.h>
#include <string.h>
#include <semaphore.h> 

void display(char *prog, char *bytes, int n);

int main(void)
{
  const char *name = "/shm-example";	// file name
  const int SIZE = 4096;		// file size
  const char *nameEmpty = "EMPTY";
  const char *nameFull = "FULL";

  int shm_fd;		// file descriptor, from shm_open()
  char *shm_base;	// base address, from mmap()
  sem_t * empty_sem, *full_sem;


  // Se abre la memoria compartida como si fuera un archivo
  shm_fd = shm_open(name, O_RDONLY, 0666);
  if (shm_fd == -1) {
    printf("cons: Shared memory failed: %s\n", strerror(errno));
    exit(1);
  }

  // Se mapea la memoria
  shm_base = mmap(0, SIZE, PROT_READ, MAP_SHARED, shm_fd, 0);
  if (shm_base == MAP_FAILED) {
    printf("cons: Map failed: %s\n", strerror(errno));
    // close and unlink?
    exit(1);
  }

    /**
     * Se abren los semaforos
     */
 
    //Se crean e inicializan los semaforos igual que en el productor
    if ( (empty_sem = sem_open(nameEmpty, 0)) == SEM_FAILED)
        printf("Error opening %s: %s\n",nameEmpty, strerror(errno));
    if ( (full_sem = sem_open(nameFull, 0)) == SEM_FAILED)
        printf("Error opening %s: %s\n",nameFull, strerror(errno));
    do {
      // Se decrementa semaforo elementosParaLeer (SemFull)
      if (sem_wait(full_sem)!=0) 
          printf("Error waiting %s\n",strerror(errno));
      else {
          /* read from the mapped shared memory segment */
          display("cons_before", shm_base, 64);	// first as bytes, then as a string
          printf("%s", shm_base);
          display("cons_after", shm_base, 64);	// first as bytes, then as a string
          //Se incrementa semaforo EspacioParaEscribir (SemEmpty)
          if (sem_post(empty_sem)!=0)
             printf("Error posting %s\n",strerror(errno));
       }
     } while (strlen(shm_base) > 0);
     sem_close(empty_sem);
     sem_unlink(nameEmpty);
     sem_close(full_sem);  
     sem_unlink(nameFull);

  //Se saca el mapeo de la memoria
  if (munmap(shm_base, SIZE) == -1) {
    printf("cons: Unmap failed: %s\n", strerror(errno));
    exit(1);
  }

  // Se cierra la memoria compartida como si fuera un archivo
  if (close(shm_fd) == -1) {
    printf("cons: Close failed: %s\n", strerror(errno));
    exit(1);
  }

  // se borra la memoria compartida
  if (shm_unlink(name) == -1) {
    printf("cons: Error removing %s: %s\n", name, strerror(errno));
    exit(1);
  }

  return 0;
}

void display(char *prog, char *bytes, int n)
{
  printf("display: %s\n", prog);
  for (int i = 0; i < n; i++)
    { printf("%02x%c", bytes[i], ((i+1)%16) ? ' ' : '\n'); }
  printf("\n");
}

